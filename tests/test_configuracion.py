"""Tests de la página Configuración y catálogo - streamlit.testing.v1.AppTest.

Renombrado desde tests/test_admin.py tras Phase 3 (Plan 03-04):
- La página de administración pasó a llamarse pages/configuracion_catalogo.py.
- Se retiraron los editores de catálogos de partidas; la creación de variantes
  vive ahora inline en la calculadora (botón "+" junto al selectbox).
- La página solo contiene porcentajes globales + defaults UI + drift.
"""
...
from src.catalogo.repositorio import cargar_todo, guardar_todo
...
def test_configuracion_carga():
    """La página de configuración carga sin excepciones y tiene 18 number_inputs.

    Composición esperada tras el recorte (Plan 03-04):
      - EXPANDER 1 (Financiero y generales): 7 inputs
        (pct_gg, pct_bi, pct_iva, factor_esponjamiento, pct_manual_defecto,
         pct_ci, conduccion_provisional_precio_m).
      - EXPANDER 6 (Valores por defecto): 11 inputs
        (aba_longitud_m, san_longitud_m, aba_profundidad_m, san_profundidad_m,
         pav_aba_acerado_m2, pav_san_calzada_m2, acometidas_n,
         pav_aba_bordillo_m, pav_san_acera_m2, pct_seguridad, pct_gestion).
    """
    at = AppTest.from_file("pages/configuracion_catalogo.py", default_timeout=10).run()
    assert not at.exception
    assert len(at.number_input) == 18, (
        f"Esperados 18 number_input, encontrados {len(at.number_input)}"
    )


def test_configuracion_valor_invalido():
...
def test_configuracion_guardar_valido():
...
def test_configuracion_secciones_principales():
    """La página renderiza las secciones que sobreviven tras el recorte (Plan 03-04).

    Tras retirar los editores de catálogos de partidas, la página solo tiene
    los expanders de "Financiero y generales" y "Valores por defecto", más
    el caption explicativo. Validamos su presencia textual."""
    at = AppTest.from_file("pages/configuracion_catalogo.py", default_timeout=10).run()
    assert not at.exception

    # Captions presentes (incluye el caption introductorio del recorte)
    captions_texto = " ".join(c.value for c in at.caption).lower()
    assert "configuración y catálogo" in (
        " ".join(t.value for t in at.title).lower()
    ), "El título 'Configuración y catálogo' debe estar presente."
    assert "boton" in captions_texto or "botón" in captions_texto or "selectbox" in captions_texto, (
        f"Caption explicativo del flujo inline no encontrado: {captions_texto[:200]}"
    )

    # Los expanders esperados están presentes en algún lugar del render.
    # AppTest no expone directamente los labels de los expanders, así que
    # comprobamos por la presencia de los inputs típicos.
    labels_inputs = {ni.label.lower() for ni in at.number_input}
    assert any("gastos generales" in lab for lab in labels_inputs), (
        "Falta input 'Gastos Generales' (financiero)."
    )
    assert any("longitud aba" in lab for lab in labels_inputs), (
        "Falta input 'Longitud ABA' (defaults UI)."
    )


def test_configuracion_cancelar_no_guarda():
...
def _configuracion_drift_critico_requiere_confirmacion_obsoleto():
    pytest.skip("La auditoria drift ya no pertenece a configuracion_catalogo.py")
...
def test_configuracion_round_trip_precio_base():
...
def test_configuracion_diff_muestra_cambios():
