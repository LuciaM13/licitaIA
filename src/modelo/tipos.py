...
class ItemCatalogo(TypedDict, total=False):
    ...
    - **Acerado / Bordillo / Calzada**: label, unidad, precio, red.
    - **Demolición**: red, label, unidad, material, precio.
...
class ResultadoPresupuesto(TypedDict, total=False):
    ...
    # contenido se genera en ``src.sistema_experto.explicaciones``).
    trazabilidad: dict[str, list[str]]
    # Cadena de inferencia del SE CLIPS (provenance: regla -> etiquetas
    # -> inputs con valor real -> fuente). Inyectada por la página tras
    # llamar al SE; calcular_presupuesto() no la calcula directamente.
    # Opcional gracias a total=False ya activo en el TypedDict:
    # presupuestos antiguos del historial cargan cadena_inferencia=[]
    # sin recompute (CONTEXT.md LD-4).
    cadena_inferencia: list["CadenaItem"]
