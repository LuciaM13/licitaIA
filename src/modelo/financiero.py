...
def calcular_resumen(
    ...
    El PEM ya incluye los materiales; ``materiales`` representa el importe que
    el caller decide excluir de la base GG/BI. Convención del Excel EMASESA:

      - **ABA** (hojas A-Fundición 150 / Fundición 300): celda SUMA
        ``=SUM(J57:J60)`` con 4 sumandos, NO incluye e)MATERIALES. El caller
        debe pasar ``materiales = importe_e_aba``.
      - **SAN** (hojas S-Hormigón 2000 / S-Gres 300): celda SUMA
        ``=SUM(J48:J52)`` con 5 sumandos, SÍ incluye e)MATERIALES. El caller
        debe pasar ``materiales = 0`` (los SAN no se excluyen). En proyectos
        mixtos, pasar solo los materiales ABA.

    Args:
        pem: Suma de todos los capítulos (incluyendo materiales).
        materiales: Importe a excluir de la base GG/BI (típicamente solo los
            materiales ABA — los SAN entran en la base por convención EMASESA).
