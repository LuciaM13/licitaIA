...
(``src.sistema_experto.motor_clips``), que clasifica el proyecto y emite avisos
...
Convención ``None`` / wildcard:
  ...
  - En los hechos CLIPS se serializa como ``"*"`` (ver ``NULL_SENTINEL``
    en ``src.modelo.constantes``). Las funciones de abajo aceptan ambas
    representaciones.
"""
...
from src.modelo.constantes import NULL_SENTINEL
from src.modelo.tipos import ItemCatalogo
