"""
Generación de explicaciones en lenguaje natural sobre las decisiones del decisor.

Estas funciones reciben los parámetros de entrada y los resultados de los
filtros de elegibilidad + desempate y producen frases en castellano listas
para mostrar al licitador.

Son funciones puras: no tienen efectos laterales, sin acceso a BD ni CLIPS.

Nota sobre el nombre
--------------------
El módulo se llama ``explicaciones.py`` porque su cometido es *explicar el
ganador* de cada desempate determinista. No traza el razonamiento de un
motor CLIPS — en este proyecto CLIPS solo emite alertas técnicas y no
participa en la selección de material.

La clave de persistencia en BD (``presupuesto_trazabilidad``) y la clave
del dict de resultado (``resultado["trazabilidad"]``) conservan su nombre
histórico para no romper el historial ya guardado.
"""
...
def generar_explicaciones(
    ...
    Devuelve una lista de strings, uno por cada decisión determinista del
    decisor (entibación, pozo, valvulería, desmontaje).
    """
    logger.debug("[EXPL-%s] Generando explicaciones: DN=%d P=%.2f inst=%s desm=%s",
    ...
    logger.debug("[EXPL-%s] Entibación: %s", red, txt_entib)
    ...
    logger.debug("[EXPL-%s] Pozo: %s", red, txt_pozo)
    ...
    logger.debug("[EXPL-%s] Valvulería: %s", red, txt_valv)
    ...
    logger.debug("[EXPL-%s] Desmontaje: %s", red, txt_desm)
