"""Use case de gestión del historial de presupuestos.

CRUD completo sobre la tabla ``presupuestos`` y sus tablas anidadas
(``presupuesto_capitulos``, ``presupuesto_partidas``,
``presupuesto_parametros``, ``presupuesto_trazabilidad``).

Nota arquitectónica — **persistencia embebida, no clean architecture pura**:
este use case conserva acceso directo a SQLite vía ``conectar()`` en vez
de delegarlo a un ``RepositorioHistorial``. Es pragmatismo consciente: la
aplicación tiene un único almacén (SQLite local) y un único adaptador
(Streamlit), por lo que introducir un puerto formal sería ceremonia sin
beneficio operativo. Si en el futuro aparece un segundo adaptador (API
REST, export a JSON externo, CLI batch…), la frontera se puede introducir
entonces sin reescribir este fichero.

Ver ADR-001 en el plan de Fase 3.
"""
...
from src.almacenamiento import conectar, _rows_to_dicts
...
def guardar_presupuesto(resultado: dict, parametros: dict[str, str],
    ...
    with conectar(path) as conn:
        try:
            ...
            # Trazabilidad (decisiones del decisor determinista; la clave
            # conserva su nombre histórico en BD para no romper el schema).
            ...
            # Cadena de inferencia (provenance del SE CLIPS, nuevo en m17).
            # Tabla relacional paralela a presupuesto_trazabilidad. Cada
            # item viene pre-formateado desde explicar_alerta(); guardamos
            # el texto literal para que el historial sea snapshot inmutable
            # (LD-4: no se recomputa al cargar).
            cadena = resultado.get("cadena_inferencia", [])
            for posicion, item in enumerate(cadena):
                conn.execute(
                    "INSERT INTO presupuesto_cadena_inferencia "
                    "(presupuesto_id, posicion, rule_id, nivel, capa, texto, fuente) "
                    "VALUES (?,?,?,?,?,?,?)",
                    (presupuesto_id, posicion,
                     str(item["rule_id"]),
                     str(item["nivel"]),
                     int(item["capa"]),
                     str(item["texto"]),
...
def obtener_presupuesto(presupuesto_id: int,
    ...
    with conectar(path) as conn:
        ...
        # Trazabilidad (decisiones del decisor determinista)
        ...
        # Cadena de inferencia (vacía para presupuestos guardados antes de m17).
        cadena: list[dict] = []
        for c_row in conn.execute(
                "SELECT rule_id, nivel, capa, texto, fuente "
                "FROM presupuesto_cadena_inferencia "
                "WHERE presupuesto_id = ? ORDER BY posicion",
                (presupuesto_id,)):
            cadena.append({
                "rule_id": c_row["rule_id"],
                "nivel": c_row["nivel"],
                "capa": int(c_row["capa"]),
                "texto": c_row["texto"],
                "fuente": c_row["fuente"],
            })
        resultado["cadena_inferencia"] = cadena
