"""Definición DDL del schema completo de la BD.

``_SCHEMA`` contiene los ``CREATE TABLE IF NOT EXISTS`` que ejecuta
``init_db`` cada arranque. Es la única fuente de verdad del schema:
cualquier cambio estructural se aplica editando este string. La
idempotencia de los ``IF NOT EXISTS`` permite ejecutarlo sobre BDs
ya existentes sin efecto alguno.
"""
...
-- Trazabilidad: cadena de inferencia formateada del SE CLIPS por presupuesto.
-- Persiste regla disparada → etiquetas de apoyo → inputs → fuente normativa.
-- Invariante de auditoría TFG: no se recomputa al cargar historial; los
-- presupuestos guardados muestran la cadena con la que se calcularon.
CREATE TABLE IF NOT EXISTS presupuesto_cadena_inferencia (
    ...
    PRIMARY KEY (presupuesto_id, posicion),
    FOREIGN KEY (presupuesto_id) REFERENCES presupuestos(id) ON DELETE CASCADE,
    CHECK (nivel IN ('alerta', 'etiqueta')),
    CHECK (capa IN (1, 2, 3))
);

CREATE INDEX IF NOT EXISTS idx_cadena_inf_rule_id
    ON presupuesto_cadena_inferencia(rule_id);
"""
