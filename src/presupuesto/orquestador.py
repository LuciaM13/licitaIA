...
No contiene fórmulas de obra (viven en ``src.modelo.geometria`` y
``src.presupuesto.capitulos_*``), no contiene reglas de elegibilidad
(viven en ``src.modelo.reglas``) y no accede a persistencia (SQLite queda
tras la frontera de ``src.almacenamiento``).
"""
...
from src.modelo.tipos import ItemCatalogo, Precios, ResultadoPresupuesto
...
def calcular_presupuesto(
    ...
    estado: dict[str, Any] = {
        "excluir_ss": 0.0,
        "materiales_total": 0.0,       # ABA + SAN — usado para base S&S/GA
        "materiales_aba_total": 0.0,   # solo ABA — usado para base GG/BI
        "materiales_san_total": 0.0,   # solo SAN — informativo (no se excluye de GG/BI)
        "trazabilidad": {},
        "aux_aba": None,
        "aux_san": None,
    }
    ...
    # técnicas vía src.sistema_experto.motor_clips). El decisor aplica elegibilidad
    ...
    # Excel EMASESA: en ABA, SUMA(a..d) excluye e)Materiales (J57:J60); en SAN,
    # SUMA(a..e) los incluye (J48:J52). LicitaIA replica esta convención
    # excluyendo de la base GG/BI únicamente los materiales ABA.
    logger.info("PEM = %.2f € (materiales ABA excluidos de GG/BI = %.2f €, materiales SAN incluidos = %.2f €)",
                pem, estado["materiales_aba_total"], estado["materiales_san_total"])
    fin = calcular_resumen(
        pem=pem,
        materiales=estado["materiales_aba_total"],
